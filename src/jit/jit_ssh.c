...
uint32_t jit_ssh(ss_program_t prog[RANDOMX_CACHE_ACCESSES], uint8_t *cache_ptr, uint8_t *buf, bool is_shared_memory, int memory_pages_of_dataset) {
	...
	WASM_SECTION(WASM_SECTION_IMPORT, {
		...
		// limit [..]
		WASM_I64(memory_pages_of_dataset);
		WASM_I64(memory_pages_of_dataset);
	});
	...
	WASM_SECTION(WASM_SECTION_CODE, {
		...
		WASM_U32_PATCH({
			...
			WASM_U8_THUNK({
				0x20, R(0), // local.get $r0
				0x20, R(1), // local.get $r1
				0x20, R(2), // local.get $r2
				0x20, R(3), // local.get $r3
				0x20, R(4), // local.get $r4
				0x20, R(5), // local.get $r5
				0x20, R(6), // local.get $r6
				0x20, R(7), // local.get $r7
			});
