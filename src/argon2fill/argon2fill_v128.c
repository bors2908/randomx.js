...
static inline v128_t THIN
fBlaMka(v128_t x, v128_t y) {
	v128_t z = wasm_mm_mul_epu32(x, y);
...
#define wasm_mm_srli_si128(__a, __imm) __extension__({ (v128_t) wasm_i8x16_shuffle((__a),                                        \
...
#define wasm_mm_slli_si128(__a, __imm) __extension__({ (v128_t) wasm_i8x16_shuffle(wasm_i64x2_const(0, 0),                      \
		                                                                           (__a),                                       \
...
#define wasm_mm_bslli_si128(__a, __imm) \
	wasm_mm_slli_si128((__a), (__imm))

#define wasm_mm_bsrli_si128(__a, __imm) \
	wasm_mm_srli_si128((__a), (__imm))

#define wasm_mm_alignr_epi8(__a, __b, __count)                                                                                                                 \
	((__count <= 16)                                                                                                                                           \
	     ? (wasm_v128_or(wasm_mm_bslli_si128((__a), 16 - (((unsigned int)(__count)) & 0xFF)), wasm_mm_bsrli_si128((__b), (((unsigned int)(__count)) & 0xFF)))) \
	     : (wasm_mm_bsrli_si128((__a), (((unsigned int)(__count)) & 0xFF) - 16)))

#define DIAGONALIZE(A0, B0, C0, D0, A1, B1, C1, D1) \
	do {                                            \
		v128_t t0 = wasm_mm_alignr_epi8(B1, B0, 8); \
		v128_t t1 = wasm_mm_alignr_epi8(B0, B1, 8); \
		B0 = t0;                                    \
		B1 = t1;                                    \
                                                    \
		t0 = C0;                                    \
		...
		C1 = t0;                                    \
                                                    \
		t0 = wasm_mm_alignr_epi8(D1, D0, 8);        \
		t1 = wasm_mm_alignr_epi8(D0, D1, 8);        \
		D0 = t1;                                    \
		D1 = t0;                                    \
	} while (0)
...
#define UNDIAGONALIZE(A0, B0, C0, D0, A1, B1, C1, D1) \
	do {                                              \
		v128_t t0 = wasm_mm_alignr_epi8(B0, B1, 8);   \
		v128_t t1 = wasm_mm_alignr_epi8(B1, B0, 8);   \
		B0 = t0;                                      \
		B1 = t1;                                      \
                                                      \
		t0 = C0;                                      \
		...
		C1 = t0;                                      \
                                                      \
		t0 = wasm_mm_alignr_epi8(D0, D1, 8);          \
		t1 = wasm_mm_alignr_epi8(D1, D0, 8);          \
		D0 = t1;                                      \
		D1 = t0;                                      \
	} while (0)
...
static void fill_block(v128_t *state, const argon2_block_t *ref_block,
	...
	if (with_xor) {
		for (i = 0; i < ARGON2_OWORDS_IN_BLOCK; i++) {
			state[i] = wasm_v128_xor(state[i], wasm_v128_load((const v128_t *)ref_block->v + i));
			block_XY[i] = wasm_v128_xor(state[i], wasm_v128_load((const v128_t *)next_block->v + i));
		}
	} else {
		for (i = 0; i < ARGON2_OWORDS_IN_BLOCK; i++) {
			block_XY[i] = state[i] = wasm_v128_xor(state[i], wasm_v128_load((const v128_t *)ref_block->v + i));
		}
	}
...
void randomx_argon2_fill_segment_v128(const argon2_instance_t *instance, argon2_position_t position) {
